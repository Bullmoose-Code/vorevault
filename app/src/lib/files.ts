...
export async function listFiles(
  ...
  if (uploaderId) {
    ...
    conds.push(`f.uploader_id = $${params.length}`);
  }
  if (rootOnly) {
    conds.push("f.folder_id IS NULL");
  }
  params.push(limit, offset);
  const dataRes = await pool.query<FileWithUploader>(
  ...
  const countConds: string[] = ["deleted_at IS NULL"];
  const countParams: unknown[] = [];
  if (uploaderId) {
    countParams.push(uploaderId);
    countConds.push(`uploader_id = $${countParams.length}`);
  }
  if (rootOnly) {
    countConds.push("folder_id IS NULL");
  }
  const countRes = await pool.query<{ count: string }>(
    `SELECT count(*)::text AS count FROM files WHERE ${countConds.join(" AND ")}`,
    countParams,
  );
  return {
    files: dataRes.rows,
...
export async function listRecentFiles(
  ...
  const where = rootOnly
    ? "f.deleted_at IS NULL AND f.folder_id IS NULL"
